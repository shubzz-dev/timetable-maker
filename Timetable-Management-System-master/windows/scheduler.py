...
def update_p(d, p, tree, parent):
    ...
    # Conflict check: is this teacher already teaching another section at the same time?
        try:
            ...
            if row[0] == 'NULL' and row[1] == 'NULL':
                conn.execute("DELETE FROM SCHEDULE WHERE ID=?", (section + str((d * periods) + p),))
                conn.commit()
                update_table()
                parent.destroy()
                return
            ...
            conflict = conn.execute(
                "SELECT * FROM SCHEDULE WHERE DAYID=? AND PERIODID=? AND FINI=? AND SECTION!=?",
                (d, p, row[0], section)
            ).fetchone()
            if conflict:
                messagebox.showerror(
                    "Conflict",
                    f"Teacher {row[0]} is already assigned in Day {day_names[d]}, Period {p + 1} (Section {conflict[4]})"
                )
                parent.destroy()
                return

            # Save entry
            conn.execute("""REPLACE INTO SCHEDULE 
                (ID, DAYID, PERIODID, SUBCODE, SECTION, FINI)
                VALUES (?, ?, ?, ?, ?, ?)""",
